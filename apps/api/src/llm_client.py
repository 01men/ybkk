"""aios_llm_client —— 统一 LLM 调用抽象（V3-V6：支持本地 ollama + Anthropic 兼容云端 API）。

通过环境变量切换：
- AIOS_LLM_PROVIDER=ollama    (默认，向后兼容)
- AIOS_LLM_PROVIDER=anthropic (Anthropic Messages 协议，OpenAI/DeepSeek/MiniMax 等可走此协议)
"""
...
logger = logging.getLogger("aios_llm_client")
...
def _to_anthropic_messages(messages: list[dict[str, str]]) -> list[dict[str, str]]:
    """去掉 system，剩下的 user/assistant 直接对应 Anthropic messages。"""
...
__all__ = ["chat", "generate", "get_provider", "get_model"]
